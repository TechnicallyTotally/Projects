		...
		function Tabs.NewTab(tabName)
				...
				function Elements.NewTextBox(argstable)
					...
					TextBox.FocusLost:Connect(function(EnterPressed)
						...
						if not EnterPressed then 
							...
							wait(0.18)
							if flag then
					       GuiLibrary.Flags[flag] = Textbox.Text
					    end
							TextBox.Text = ""  
						end
					end)
          ...
          local DefaultScale = (defaultvalue - minvalue) / (maxvalue - minvalue)
				...
				function Elements.NewDropdown(argstable)
					...
					local defaultvalue = argstable["Default"]["Value"] or nil
					local flag = argstable["Flag"] or nil
					local callback = argstable["Function"]
					local DropFunction = {}
          ...
          if defaultvalue then
            task.spawn(callback, defaultvalue)
            pcall(function()
              itemTextbox.Text = defaultvalue
            end)
            if flag then
              GuiLibrary.Flags[flag] = defaultvalue
            end
          end
					...
					for i,v in next, list do
						...
						optionSelect.MouseButton1Click:Connect(function()
							if not focusing then
								...
								itemTextbox.Text = v
								if flag then
								  GuiLibrary.Flags[flag] = itemTextbox.Text
								end
								dropFrame:TweenSize(UDim2.new(0, 352, 0, 33), 'InOut', 'Linear', 0.08)
								wait(0.1)
								updateSectionFrame()
